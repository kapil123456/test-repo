...
if(isset($_SESSION['er']) && $_SESSION['er']!="" && !isset($_POST['submitButton']))
{
...
if(isset($_POST) && isset($_POST['submitButton']) && $_POST['submitButton'])
{
...
// Display number of fetched files status 
  for($n=0;$n<count($data);$n++)
 {
 $iqry1="SELECT count(f_id) as rcrd_count from tblfetch where f_pmrn='".$data[$n]['p_mrn']."'";
$flag1=$con->getSingleRow($iqry1,  __FILE__,  __LINE__);
 
 $iqry2="SELECT * from tblfetch where f_pmrn='".$data[$n]['p_mrn']."'";
$flag2=$con->getSingleRow($iqry2,  __FILE__,  __LINE__);
 
  
  //get sum of count from tbl Visits
$vcqry1="SELECT sum(v_fcount) as v_fcount from tblvisits where v_mrn='".$data[$n]['p_mrn']."'";
$fcount1=$con->getSingleRow($vcqry1,  __FILE__,  __LINE__);


//get count from tbl files
$vfcqry1="SELECT count(f_filename) as f_count from tblfile where p_id='".$data[$n]['p_id']."'";
 $vfcount1=$con->getSingleRow($vfcqry1,  __FILE__,  __LINE__);
  
	// Logic for Immediate status
	//checks the v_count(tblvisits) for records(0) and check the f_reqstatus for not OK then set  Status to Immediate 
   if($flag1['rcrd_count'] == 0 ) 
   {	         
	   $data[$n]['p_type'] = 'Needs Retrieval';
    }  
else 
{	  
 if($fcount1['v_fcount']==$vfcount1['f_count'] && $flag2['f_reqstatus'] == "OK")
 { 				
		//compare both counts for equality and check the f_reqstatus as OK then  set Status to Immediate 
		$data[$n]['p_type'] = 'Immediate';
  }
 else
    {
	    $data[$n]['p_type'] = 'Processing';
    }
 
  }
}
 //END logic for displaying no.of files fetched and Immediate status logic
}
...
function Clickimage(obj) {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#dialog4").html(data);
	  $("#dialog4").dialog({model:true, width:600}).dialog('open');
    }
  });
...
function checkProgress(obj) {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#dialog3").html(data);
	  $("#dialog3").dialog({model:true, width:200,height:200}).dialog('open');
    }
  });
 ...
 function fetchFiles(obj) {
  ...
  $.ajax({
  	type:"post",
    url: 'fetch - automation.php',
	data: {p_mrn:$(obj).attr('value')},
    success: function(data) {

    //  $("#dialog3").html(data);
	//  $("#dialog3").dialog({model:true, width:300,height:150}).dialog('open');
    	...
    	<div id="Header">
            ...
            <div id="Head-right">
                ...
                <div id="Hr-row-2">
					...
					<?php }?>
					<!-- changes to include fetch button -->
					<div id="Hr-row2-c3">
							<a href="erase.php" class="button" style="width:auto" > Fetch</a>
					</div>
					<!-- end fetch button -->
...
<!--    	<select  id="pfiltermrn" name="pfiltermrn" class="Filling-option">
  ...
  <tr>
    <td><input id="submitButton" name="submitButton" type="submit" value="Search" class="Logout-btn" /></td>
...
<table id="tblResults" width="961" border="0" cellspacing="0" cellpadding="5">
  <tr class="t-head">
   <td width="28" class="border"><input id="selectall"   name="selectall" type="checkbox" value="" style="display:none"/></td>
    ...
    <td width="115" class="border">MRN</td>
     <td width="106" class="border">Status </td>
 ...
 ?><tr class="<?php echo ($i%2!=0 ? "t-line-white" : "t-line-grey");?>">
    ...
    <td> <?php if( $data[$i]['p_type'] == 'Immediate'){?> <a id="opener<?php echo $data[$i]['p_id'];?>" href="#" value="<?php echo $data[$i]['p_id'];?>" onClick="getFiles(this);">View Files</a> <a href="#" onClick="Clickimage(this);"><img src="images/certification-icon.gif" style="width:20%; height:auto;"></a><?php }
	   if( $data[$i]['p_type'] == 'Processing' ){?> <a id="opener<?php echo $data[$i]['p_id'];?>" href="#" value="<?php echo $data[$i]['p_mrn']?>" onClick="checkProgress(this)">Check Progress</a>
	 <?php } if( $data[$i]['p_type'] == 'Needs Retrieval' ) {?> <a href="#" value="<?php echo $data[$i]['p_mrn']?>" onClick="fetchFiles(this)">Fetch File </a><?php }?></td>
  ...
  <?php } ?>
                                         ...
                                         <div id="dialog3" title="Information"></div>
                                        <div id="dialog4" title="Certification"></div>
										<div class="clear"></div>                
                </div>
			</div>
