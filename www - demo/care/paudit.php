...
if(isset($_REQUEST['_cmd'])){
  ...
  $atype='Patient Audit';
  $pt='';
  $details="Viewed Patient Audit";
  $ip=getIP();
  $date=date("Y-m-d H:i:s");
  $dv="paudit.php";
  $k= useraudit($con,$date,$ip,$us,$atype,$pt,$details,$dv);
  header("location: paudit.php");
  exit();
}
...
$qy="SELECT count(id) as count FROM tbllog where patient<>''";
...
{
$arry=array('txt_P_Name'=>'patient');
$keys=array_keys($arry);
 $searchfilter.=' '. $arry[$keys['0']]." like('%".$_POST[$keys['0']]."%') and";

}

$qry="SELECT * FROM tbllog where  ".($searchfilter!=''? $searchfilter:'')."  patient<>''  order by id DESC LIMIT ".$offset.",".$rec_limit;
...
<table id="tblResults" width="961" border="0" cellspacing="0" cellpadding="5">
  <tr class="t-head">
   <td width="28" class="border"><input id="selectall"   name="selectall" type="checkbox" value="" style="display:none"/></td>
    ...
    <td width="100" class="border">Patient</td>
    <td width="115" class="border">Document Viewed</td>
    <td width="115" class="border">User</td>
  </tr>
 ...
 <tr class="<?php echo ($i%2!=0 ? "t-line-white" : "t-line-grey");?>">
  ...
  <?php } ?>
            	...
            	<div id="Rsults-btn-row">
                	<div id="View-btn-div"><input name="Email" type="button" value="Email" onClick="document.location.href='email.php?d1=paudit';" class="Logout-btn" /></div>
                    <div id="Prv-nxt-Main">
