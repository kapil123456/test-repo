...
<table id="tblResults" width="961" border="0" cellspacing="0" cellpadding="5">
 ...
 ?><tr class="<?php echo ($i%2!=0 ? "t-line-white" : "t-line-grey");?>">
	...
	<td><?php echo $data[$i]['f_id'];?></td>
	<td><?php echo $data[$i]['f_pmrn'];?></td>
    <td>
	<?php
	
	//start Immediate logic
	//$testqry="SELECT f_reqstatus FROM tblfetch WHERE f_id=(Select f_id FROM tblfetch WHERE f_pmrn=@p_mrn) AND f_reqstatus='OK' ".($searchfilter!='' ? 
//	" where ".$searchfilter:'');
	//
//	$testqry="SELECT f_reqstatus FROM tblfetch WHERE ".($searchfilter!=''? $searchfilter:'')."  patient<>''  order by f_pmrn DESC LIMIT ";
	//
//	$rc=$con->getMultiRow($testqry,  __FILE__,  __LINE__);
	//if( $data[$i]['p_type'] == 'Immediate'){
	//end Immediate logic
	?>
	<a id="opener<?php echo $data[$i]['p_id'];?>" href="#" value="<?php echo $data[$i]['p_id'];?>" onClick="getFiles(this);">View Files</a>
	<?php }
